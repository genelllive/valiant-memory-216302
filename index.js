...
function handleSwitch(panoImg, loc, pano, index) {
  ...
  if (!loc.prev_heading) {
    ...
    var time = getTimeForNextDistance();
    console.log('timeout for : ' + time + 's');
    setTimeout(function() {
      animateNext();
    }, (time * 1000));
...
function resolvePanoIds(routePolyline) {
  ...
  $.get('https://maps.googleapis.com/maps/api/streetview/metadata?location=' 
          + routeLocations[loadedIndex].toUrlValue() + '&key=AIzaSyDnBYs-Vi73pxKx0ZopztUVmL7kvff4rLM', function(data) {
